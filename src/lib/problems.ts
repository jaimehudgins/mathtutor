...
const problemGenerators: Record<string, () => Problem> = {
  ...
  '7-rp-3': () => {
    ...
    return {
      ...
      acceptableAnswers: [final.toString(), `$${final}`, final.toFixed(2), `$${final.toFixed(2)}`],
  ...
  '7-ns-2': () => {
    ...
    const b = randChoice([-6, -5, -4, -3, -2, 2, 3, 4, 5, 6].filter(n => n !== 0));
    const operation = randChoice(['×', '÷']);
    let result: number;
    let question: string;
    const aStr = a < 0 ? `(${a})` : a.toString();
  ...
  '7-ee-1': () => {
    ...
    return {
      ...
      acceptableAnswers: [`${xCoef}x + ${constant}`, `${xCoef}x+${constant}`, `${constant} + ${xCoef}x`],
  ...
  '7-g-4': () => {
    ...
    return {
      ...
      acceptableAnswers: [approxAnswer.toString(), answer.toString(), approxAnswer.toFixed(2), answer.toFixed(2)],
      hint: type === 'area'
  ...
  '7-g-5': () => {
    // Angle relationships
    const type = randChoice(['supplementary', 'complementary']);
    const angle1 = type === 'supplementary' ? randInt(30, 150) : randInt(10, 80);
    const angle2 = type === 'supplementary' ? 180 - angle1 : 90 - angle1;

    return {
      id: Date.now().toString(),
      standardId: '7-g-5',
      question: `Two angles are ${type}. One angle measures ${angle1}°. What is the measure of the other angle?`,
      correctAnswer: angle2.toString(),
      acceptableAnswers: [angle2.toString(), `${angle2}°`, `${angle2} degrees`],
      hint: type === 'supplementary'
        ? 'Supplementary angles add up to 180°.'
        : 'Complementary angles add up to 90°.',
      explanation: type === 'supplementary'
        ? `Supplementary angles sum to 180°. So ${angle1}° + x = 180°, x = ${angle2}°`
        : `Complementary angles sum to 90°. So ${angle1}° + x = 90°, x = ${angle2}°`,
      difficulty: 'easy'
    };
  },
  ...
  '7-g-6': () => {
    ...
    return {
      ...
      acceptableAnswers: [volume.toString(), `${volume} cm³`, `${volume} cubic cm`],
  ...
  '7-sp-5': () => {
    ...
    return {
      ...
      acceptableAnswers: [`${num}/${den}`, `${favorable}/${total}`, decimal.toString()],
  ...
  '7-sp-8': () => {
    ...
    const coin = randChoice(['heads', 'tails']);
    const dieNum = randInt(1, 6);

    return {
      id: Date.now().toString(),
      standardId: '7-sp-8',
      question: `You flip a coin and roll a die. What is the probability of getting ${coin} AND rolling a ${dieNum}? Express as a fraction.`,
      correctAnswer: '1/12',
      acceptableAnswers: ['1/12', '0.083', '0.08'],
      hint: 'For independent events, multiply the individual probabilities.',
      explanation: `P(${coin}) = 1/2 and P(rolling ${dieNum}) = 1/6. P(both) = 1/2 × 1/6 = 1/12`,
      difficulty: 'medium'
    };
  },
};
...
export function getProblemForWeakArea(progress: { standardId: string; masteryLevel: number }[]): Problem {
...
export function checkAnswer(problem: Problem, userAnswer: string): boolean {
  const normalized = userAnswer.trim().toLowerCase().replace(/\s+/g, '').replace('$', '');
  const correctNormalized = problem.correctAnswer.toLowerCase().replace(/\s+/g, '');
